...
export class Database {
  ...
  async deactivateGiveaway(giveawayId: string): Promise<void> {
    const query = 'UPDATE giveaways SET is_active = FALSE WHERE id = $1';
    await this.pool.query(query, [giveawayId]);
  }

  async getLatestGiveaway(): Promise<GiveawayData | null> {
    const query = 'SELECT * FROM giveaways ORDER BY created_at DESC LIMIT 1';
    const result = await this.pool.query(query);
    if (result.rows.length === 0) return null;
    return {
      id: result.rows[0].id,
